...
public class VROwner {
    ...
    public static class VFAddEditOwner {
        ...
        //label conditional on address based on first name
        @Label("First Name")
    	@TextBox
    	@NotNull
    	@Path
    	@LabelConditional(targetPath = "/../address", condition = {
    			@LabelConditional.Condition(when = "state != null", then = @Label("<!/!>'s address"))
    		})
